...
# ============================================================
PAGES = [
    # (filename, title, type, badge_text)
    ...
    ("rd-03.html", "热点强化3 Na₂CO₃、NaHCO₃含量测定的思维方法", "rd", "热点强化3"),
    ...
    ("rd-05.html", "热点强化5 SO₂的测定及治理", "rd", "热点强化5"),
...
# 构建查找表
IDX_MAP = {filename: idx for idx, (filename, _, _, _) in enumerate(PAGES)}
...
def make_css(scheme):
    return f"""        :root {{
            ...
            --accent-border: {scheme['accent_border']};
            --badge-bg: {scheme['badge_bg']};
            --navy-dark: #0f172a;
            --navy-mid: #1e3a5f;
            --gold: #fbbf24;
            --bg: #f0f4f8;
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .block-head {{
            padding: 16px 22px;
            background: {scheme['block_head_bg']};
        ...
        .hidden-content.open {{ display: block; }}
...
def make_page(filename, title, page_type, badge_text, prev_info, next_info, body_content=""):
    ...
    # 构建页头
    header_html = f"""    <header class="page-header">
    ...
    if prev_info:
        ...
        prev_html = f'        <a href="./{prev_file}" class="nav-prev">← 上一讲 | {prev_title}</a>'
    else:
    ...
    if next_info:
        ...
        next_html = f'        <a href="./{next_file}" class="nav-next">下一讲 | {next_title} →</a>'
    else:
        next_html = '        <a class="nav-next disabled">已是最后一讲 →</a>'

    nav_footer_html = f"""    <nav class="nav-footer">
{prev_html}
{next_html}
    </nav>"""
...
function toggleCollapsible(btn, targetClass) {{
  var target = btn.parentElement.querySelector('.' + targetClass);
  if (!target) target = btn.closest('.sub').querySelector('.' + targetClass);
  if (!target) return;
  if (target.classList.contains('open')) {{
    target.classList.remove('open');
    btn.textContent = btn.dataset.showText || '展开';
  }} else {{
    target.classList.add('open');
    btn.textContent = btn.dataset.hideText || '收起';
  }}
}}
    </script>
</body>
...
def extract_content(html):
    ...
    patterns = [
        ...
        r'<div class="main">(.*?)</div>\s*(?:<!-- 翻页导航|<!-- 底部|<!--\s*底部导航|<div class="nav-footer|<nav class="nav-footer)',
    ...
    for pattern in patterns:
        ...
        if m:
            ...
            # 清理内容末尾可能残留的旧导航
            # 去掉从 <!-- 翻页导航 或 <div class="nav-footer 开始的内容
    ...
    # Fallback: try to find content between old header end and old footer start
    m = re.search(r'(?:class="(?:main|wrap|content)"[^>]*>)(.*?)(?:<!--\s*(?:翻页导航|底部导航)|<div class="(?:nav-footer|page-footer)"|<footer class="(?:footer|site-footer)")', html, re.DOTALL)
    if m:
